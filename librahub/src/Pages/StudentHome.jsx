...
import { FaHeart, FaSearch, FaFilter, FaStar } from "react-icons/fa";
...
const StudentHome = () => {
  ...
  const [sortByHighestRated, setSortByHighestRated] = useState(false);
  ...
  useEffect(() => {
    const fetchBooksAndFavs = async () => {
      try {
        ...
        setBookAverageRatings(averages);
  ...
  const handleSortByRating = () => {
    if (sortByRating) {
      ...
      setSortByHighestRated(false);
    }
  };

  const handleSortByHighestRated = () => {
    ...
    <>
      ...
      <div className="home-container">
        ...
        <div className="search-filter-wrapper">
          ...
          <button
            className={`sort-rating-btn ${sortByHighestRated ? "active" : ""}`}
            onClick={handleSortByHighestRated}
          >
            ⭐ Highest Rated
          </button>
        </div>
                  ...
                  >
                    {[1, 2, 3, 4, 5].map((s) => (
                      <FaStar
                        ...
                        color={
                          s <=
                          Math.round(bookAverageRatings[book.id]?.average || 0)
                            ? "#FFD700"
                            : "#ddd"
                        }
                      />
                    ...
                    <span>
                      {bookAverageRatings[book.id]?.average
                        ? bookAverageRatings[book.id].average.toFixed(1)
                        : "0.0"}
                    </span>
                  </div>
